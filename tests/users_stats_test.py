'''
This file contains tests for the implementation of users stats and its http wrap.
'''
import pytest
import requests
...
def test_no_utilization(clear, new_user):
    ''' Test when user has not joined anything. '''
    response = requests.get(f"{BASE_URL}/users/stats/v1?token={new_user['token']}")
    assert response.status_code == 200
    assert len(response.json()['workspace_stats']['channels_exist']) == 1
    assert response.json()['workspace_stats']['utilization_rate'] == 0

def test_create_channel(clear, new_user):
    ''' Test correct function return for after creating a channel. '''
...
def test_one_channel_join_and_leave(clear, new_user, new_user2):
    ''' Test function returns correct list of dictionaries for when user joins and leaves a channel. '''
    chan_create_data = {'token': new_user['token'], 'name': 'Apple', 'is_public': True}
    ...
    response = requests.get(f"{BASE_URL}/users/stats/v1?token={new_user2['token']}")
    assert response.status_code == 200
    assert len(response.json()['workspace_stats']['channels_exist']) == 2
    assert response.json()['workspace_stats']['channels_exist'][-1]['num_channels_exist'] == 1
    assert response.json()['workspace_stats']['utilization_rate'] == 0.5

def test_channel_invite(clear, new_user, new_user2):
    ''' Test correct function return for when user invited to a channel. '''
    chan_create_data = {'token': new_user['token'], 'name': 'Apple', 'is_public': True}
    chan_create_resp = requests.post(f"{BASE_URL}/channels/create/v2", json = chan_create_data)
    c_id = chan_create_resp.json()['channel_id']
...
def test_one_dm_create_and_leave(clear, new_user, new_user2):
    ...
    user2_response = requests.get(f"{BASE_URL}/users/stats/v1?token={new_user2['token']}")
    assert user2_response.status_code == 200
    assert len(user2_response.json()['workspace_stats']['dms_exist']) == 2
    assert user2_response.json()['workspace_stats']['dms_exist'][-1]['num_dms_exist'] == 1
    assert user2_response.json()['workspace_stats']['utilization_rate'] == 0.5

    user1_response = requests.get(f"{BASE_URL}/users/stats/v1?token={new_user['token']}")
    assert user1_response.status_code == 200
    assert len(user1_response.json()['workspace_stats']['dms_exist']) == 2
    assert user1_response.json()['workspace_stats']['dms_exist'][-1]['num_dms_exist'] == 1
    assert user1_response.json()['workspace_stats']['utilization_rate'] == 0.5
